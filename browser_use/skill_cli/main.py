...
""",
	...
	parser.add_argument(
		'--browser',
		'-b',
		choices=['chromium', 'real', 'remote', 'safari'],
		default='chromium',
		help='Browser mode',
	)
...
def handle_profile_sync(args: argparse.Namespace) -> int:
	...
	async def sync_cookies() -> tuple[int, str | None]:
		...
		try:
			...
			await asyncio.to_thread(cookies_file.write_text, json.dumps(cookies, indent=2))
	...
	async def upload_cookies(cookies_path: str) -> tuple[int, str | None]:
		...
		try:
			# Load cookies
			cookies_raw = await asyncio.to_thread(Path(cookies_path).read_text)
