...
class SessionServer:
	...
	async def run(self) -> None:
		...
		if sock_path.startswith('tcp://'):
			...
			if await asyncio.to_thread(sock_file.exists):
				await asyncio.to_thread(sock_file.unlink)
...
def main() -> None:
	...
	parser.add_argument('--browser', default='chromium', choices=['chromium', 'real', 'remote', 'safari'])
